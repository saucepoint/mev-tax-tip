...
import {BaboonHarness} from "./harness/BaboonHarness.sol";

contract MEVTaxTestBaboonTest is Test, Fixtures {
    ...
    function setUp() public {
        ...
        deployCodeTo("BaboonHarness.sol:BaboonHarness", constructorArgs, flags);
        hook = BaboonHarness(flags);
    ...
    function test_runningAverage() public {    
        // base fee 1 gwei
        vm.fee(1 gwei);
        ...
        // 10 trades, starting at 1.1 gwei priority fee, and decreasing by 0.1 gwei each time
        for (uint256 i = 0; i < 10; i++) {
            // set the priority fee
            vm.txGasPrice(1 gwei + (1.1 gwei - (i * 0.1 gwei)));
            swap(key, zeroForOne, amountSpecified, ZERO_BYTES);
        }

        // running average is equal to the average of the bottom 8 swaps [0.1 gwei, 0.2 gwei, 0.3 gwei, 0.4 gwei, 0.5 gwei, 0.6 gwei, 0.7 gwei, 0.8 gwei]
        uint256 expectedAverage = (0.1 gwei + 0.2 gwei + 0.3 gwei + 0.4 gwei + 0.5 gwei + 0.6 gwei + 0.7 gwei + 0.8 gwei) / 8;
        uint256 calculatedAverage = hook.getRunningAverage();
        assertEq(calculatedAverage, expectedAverage);
    }
